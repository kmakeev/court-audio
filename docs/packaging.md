...
### Провижининг операторского профиля зала (B-001, этап 13.6)

Нужен **только** для изолированных залов с включённым флагом
`auth.operator.autonomous_offline.enabled` (автономный офлайн-старт по PIN без
онлайн-входа). На первом запуске (если блоб ещё не задан) ядро засеивает
операторский профиль в зашифрованный `operator_profile.enc` (ключ станции, как
