...
## Улучшения после v1 — рекомендации и промты (2026-07-02)

Собраны рекомендации по развитию — [`recommendations.md`](recommendations.md)
(проигрыватель с метками, экспорт папка/DVD, авторизация через `ex_system`,
разнесение настроек админ/оператор, адаптивность, UX-пакет). По ним оформлены
промты `10_1_playback` … `10_6_ux_polish`. Этапы `11`/`12` остаются
перспективой; серверный `07` дозавершается в `ex_system`.
