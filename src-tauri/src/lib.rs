...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let handle = app.handle().clone();
            // Env-провижининг автономного офлайн-режима (B-001, замечание станции
            // 2026-07) — ДО чтения настроек: в изолированном зале флаг негде
            // включить через UI (экран «Администрирование» за гейтом входа), а
            // прочий провижининг станции уже идёт из env. Пишет settings.json
            // только при его отсутствии и COURT_AUDIO_AUTONOMOUS_OFFLINE=1.
            match ipc::seed_autonomous_offline_from_env_if_absent(&handle) {
                Ok(true) => eprintln!(
                    "ИНФО: автономный офлайн-режим включён из env {} — записаны \
                     дефолтные настройки станции (см. docs/first_run_offline.md).",
                    ipc::AUTONOMOUS_OFFLINE_ENV
                ),
                Ok(false) => {}
                Err(e) => eprintln!(
                    "ВНИМАНИЕ: не удалось провизионировать автономный режим из env: {e}"
                ),
            }
