...
//! at-rest наследуется от основного места (R-013, этап 13.7: consumer зовёт
//! зеркало **после** финализации сегмента, поэтому при `storage.encrypt_at_rest`
//! на зеркало едет тот же `.enc`-блоб) — зеркало своего криптопути не имеет и
//! plaintext поверх шифрованного не пишет.
